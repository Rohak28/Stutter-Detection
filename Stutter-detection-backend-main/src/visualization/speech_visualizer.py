...
import numpy as np
import matplotlib
matplotlib.use('Agg')  # Must be set before importing pyplot for headless Docker
...
class SpeechVisualizer:
    ...
    def generate_spectrogram_only(
        self, audio_data: np.ndarray, sample_rate: int, filepath: str
    ) -> bool:
        """
        Generate a standalone spectrogram image for SLP verification.
        
        Args:
            audio_data: Raw audio waveform data.
            sample_rate: Audio sample rate.
            filepath: Path to save the spectrogram image.
        ...
        try:
            ...
            ax.set_title("Speech Spectrogram Analysis", fontsize=14, fontweight='bold')
            ax.set_xlabel("Time (seconds)")
            ...
            plt.close(fig)
            logger.info(f"Standalone spectrogram saved: {filepath}")
            return True
            
        except Exception as e:
            logger.error(f"Error generating standalone spectrogram: {e}")
            return False